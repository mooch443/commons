...
namespace pixel {
    ...
    pv::BlobPtr threshold_get_biggest_blob(pv::BlobWeakPtr blob, int threshold, const Background* bg, uint8_t use_closing = 0, uint8_t closing_size = 2);
    std::vector<pv::BlobPtr> threshold_blob(pv::BlobWeakPtr blob, int threshold, const Background* bg, const Rangel& size_range = Rangel(-1, -1));
    std::vector<pv::BlobPtr> threshold_blob(CPULabeling::ListCache_t&, pv::BlobWeakPtr blob, int threshold, const Background* bg, const Rangel& size_range = Rangel(-1,-1));
