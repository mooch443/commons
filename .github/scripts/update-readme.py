...
def update_code_block(file_path, marker, ticks='```'):
    ...
    pattern = re.compile(f"({marker}.*?{ticks}\\n)(.*?)(```)", re.DOTALL)
...
if __name__ == "__main__":
    update_code_block('examples/simple_gui.cpp', 'Example: Dynamic GUI', '```cpp')
    update_code_block('examples/test_gui.json', 'test_gui.json', '```json')
